...
<table align="center" style="width: 900px">	<tr>
		...
		<br />
				<br /><br />
		There is now a new feature. It is called Network Time Protocal or "NTP" 
		for short. It will use an NISP Time Server IP address set into the Web 
		interfaces NETWORK page to get&nbsp; time and date from.  You must
		set this up on the <a href="../web/web-NTP.php" target="_blank">NETWORK WEB Page</a> for this to work. 
		Please read the warning if you intend to use the local computer as the 
		NISP.&nbsp;&nbsp; After setting up the IP address on the Web page, 
		Hitting the "NTP"button will get the time in UTC and Date in UTC, if 
		using a Internet based NISP server (Recommended). It can take up to two 
		seconds for this to happen, but the Gemini-2 times will be set to within 
		1 sec.<br />
		Also if a valid NISP server address is in the WEB Network page, the 
		Gemini-2 will try and use that to get the current date and time. <br />
		<br />
		<br />
		</td>
